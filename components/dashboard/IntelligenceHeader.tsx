...
import { Info, CheckCircle2, AlertCircle, TrendingUp, Sparkles, RefreshCcw, Palette, UserCheck } from "lucide-react";
...
export function IntelligenceHeader({ report, onAdjust, onThemeChange, onReRun }: IntelligenceHeaderProps) {
    ...
    const isRefined = report.isUserRefined;
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="lg:col-span-2 bg-zinc-900/30 backdrop-blur-sm border border-zinc-800 rounded-3xl p-8 relative overflow-hidden flex flex-col justify-between">
                    ...
                    <div className="relative z-10">
                        ...
                        <div className="text-zinc-400 text-lg leading-relaxed max-w-2xl mb-8 border-l-2 border-lime-400/30 pl-6 space-y-2">
                            <p>{report.primary.explanation}</p>
                            {report.primary.evidence && report.primary.evidence.length > 0 && (
                                <p className="text-xs font-mono text-zinc-600 uppercase tracking-tighter">
                                    Evidence: {report.primary.evidence.join(' / ')}
                                </p>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="relative z-10 space-y-6">
                        ...
                        <div className="pt-6 border-t border-zinc-800/50">
                            <div className="flex flex-col md:flex-row md:items-center gap-6">
                                <div className="flex-1">
                                    <span className="text-[10px] font-mono text-zinc-600 uppercase tracking-widest block mb-3">Refine Persona</span>
                                    <div className="flex gap-2 p-1 bg-zinc-950 rounded-lg border border-zinc-900 overflow-x-auto no-scrollbar max-w-full">
                                        {ALL_ROLES.map(role => (
                                            <button
                                                key={role}
                                ...
                                <div>
                                    <span className="text-[10px] font-mono text-zinc-600 uppercase tracking-widest block mb-3">Interface Theme</span>
                                        ...
                                        {THEMES.map(t => (
                                            <button
                                                ...
                                                className={`w-8 h-8 rounded-full border-2 transition-all ${report.primary.theme === t.id
                                                    ? "border-lime-400 scale-110 shadow-[0_0_15px_rgba(163,230,53,0.3)]"
                        ...
                        <div className="flex flex-wrap gap-4">
                            {!isRefined ? (
                                <button className="px-6 py-2.5 bg-lime-400 text-black font-bold rounded-full text-sm hover:scale-105 transition-transform flex items-center gap-2">
                                    <CheckCircle2 className="w-4 h-4" />
                                    Accept Identity
                                </button>
                            ) : (
                                <button className="px-6 py-2.5 bg-zinc-800 text-white font-bold rounded-full text-sm hover:scale-105 transition-transform flex items-center gap-2 border border-zinc-700">
                            ...
                            <button
                                onClick={onReRun}
                                className="px-6 py-2.5 bg-transparent text-white font-bold rounded-full text-sm hover:bg-zinc-800 transition-colors flex items-center gap-2 border border-zinc-800"
                            >
                ...
                <div className="space-y-4">
                    {/* Secondary Persona - Only show if not fully refined or if secondary is strong */}
                    {report.secondary && (
                        <div className="bg-zinc-900/50 border border-zinc-800 rounded-2xl p-6 hover:border-zinc-700 transition-colors group cursor-pointer" onClick={() => onAdjust(report.secondary!.role)}>
                            <div className="flex items-center justify-between mb-4">
                                <span className="text-xs font-mono uppercase tracking-widest text-zinc-500">Secondary Path</span>
                    ...
                    {report.emerging && (
                        <div className="bg-lime-400/5 border border-lime-400/20 rounded-2xl p-6">
                            <div className="flex items-center gap-2 mb-4">
                                ...
                                <span className="text-xs font-mono uppercase tracking-widest text-lime-400">Neural Trend</span>
                            </div>
                                ...
                                <div key={name}>
                                    ...
                                    <div className="w-full h-1 bg-zinc-900 rounded-full overflow-hidden">
                                        <motion.div
                                            ...
                                            className="h-full bg-lime-400 opacity-50"
                                        />
                                    </div>
                                </div>
                            ))}
